...
resource "google_compute_instance" "server" {
  ...
  metadata_startup_script = templatefile("zabbix_server_provision.sh", {name = "${var.name}"})
  network_interface {
    network = google_compute_network.vpc.name
...
resource "google_compute_instance" "client" {
  ...
  metadata_startup_script = templatefile("zabbix_client_provision.sh", {server_ip = "${google_compute_address.internal_server_address.address}"})
  network_interface {
    network = google_compute_network.vpc.name
    subnetwork = google_compute_subnetwork.public.name
    access_config {
